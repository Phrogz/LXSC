...
Providing functions for the following special keys allows you to keep track of what the machine is doing:
...
…and you can run arbitrary code (just like a `<script>` block does) by:
...
You can also ask for a list of all state IDs (including those autogenerated for states that have no `id="…"` attribute):
...
With no modifications, when LXSC encounters such an executable it fires a `error.execution.unhandled` event internally with the `_event.data` set to the string `"unhandled executable type explode"`. This does not halt execution of the intepreter unless the state machine reacts to that event in a violent manner.

If you want such elements to do something more powerful, you can extend LXSC like so:
